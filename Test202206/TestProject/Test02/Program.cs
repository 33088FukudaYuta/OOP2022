...
namespace Test02 {
    class Program {
        ...
        private static void Exercise02(List<int> numbers) {
            var maxnum = numbers.Where(n => n % 2 == 0).Max();
            Console.WriteLine(maxnum);
        }
        //問題３　昇順に並べて表示（遅延実行とする）
        //　　　　出力結果【12 14 17 20 31 35 40 48 53 76 87 91 94】
        private static void Exercise03(List<int> numbers) {
            var ascnum = numbers.OrderBy(n => n);
            foreach (var ascnums in ascnum) {
                Console.Write(ascnums + " ");
            }
        }
        ...
        private static void Exercise04(List<int> numbers) {
            var betweennum = numbers.Where(n => n >= 10 && n <= 50);
            foreach (var nums in betweennum) {
                Console.Write(nums + " ");
            }
        }
    }
